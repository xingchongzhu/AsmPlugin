...
import com.data.transform.AutoTransform
import com.data.bean.AutoClassFilter
import org.gradle.api.Plugin
import org.gradle.api.Project
import com.android.build.gradle.AppExtension
import com.data.util.Logger
import com.data.bean.AutoSettingParams
import com.data.util.AutoTextUtil
...
class TransformPlugin implements Plugin<Project> {
    ...
    void apply(Project project) {
        project.extensions.create('autosettingparams', AutoSettingParams)
        GlobalConfig.setProject(project)
        def name = GlobalConfig.getParams().name
        println "Hello TransformPlugin $name"
        ...
        project.afterEvaluate {
            Logger.setDebug(project.autosettingparams.isDebug)
            // 用户配置解析
            analysiUserConfig()
        }
    }
